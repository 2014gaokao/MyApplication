...
void SplitRender::onSurfaceCreated() {
    EglHelper eglHelper = *new EglHelper();
    program = eglHelper.createProgram(VERTEX_SHADER, FRAG_SHADER_SPLIT);
    glBindAttribLocation(program, 0, "a_position");
    glBindAttribLocation(program, 1, "a_texCoord");
}

void SplitRender::onSurfaceChanged(int width, int height) {
    glViewport(0, 0, width, height);
}

void SplitRender::onDrawFrame(int textureId) {
